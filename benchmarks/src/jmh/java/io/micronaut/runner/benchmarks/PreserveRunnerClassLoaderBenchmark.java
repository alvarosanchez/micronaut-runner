...
/**
 * Measures fresh runner class loaders against a matching PRESERVE archive registration.
 *
 * <p>This variant has its own fork so the once-per-JVM handler can register the PRESERVE archive rather
 * than inheriting the STORED variant's source. Registration and sanity checks stay outside measurement;
 * archive opening, index validation, loader construction and class definition remain timed.</p>
 */
...
public class PreserveRunnerClassLoaderBenchmark {
    ...
    /** Opens the PRESERVE registration outside measurement and keeps it alive for this trial. */
    @Setup(Level.Trial)
    public void setUp() throws IOException, ClassNotFoundException {
        archive = SyntheticArchive.shared();
        names = archive.spreadSample(CLASSES);
        RunnerRegistration open = RunnerRegistration.open(archive.preserveRunnerJar());
        try {
            ClassLoaderBenchmarkSanity.verifyRunnerArchive(archive.preserveRunnerJar(), names[0]);
            registration = open;
        } finally {
